...
import { getFieldNumbers } from '../components/utils';
...
const FormEdit = memo(
  ({
    ...
    const [selectedFieldp, setSelectedField] = useState<
      IFlatCustomFieldWithIndex | undefined
    >(undefined);
    const [successMessageIsVisible, setSuccessMessageIsVisible] =
      useState(false);
    ...
    const { data: rawFormCustomFieldso, isFetching } = useFormCustomFields({
      projectId,
      phaseId: isFormPhaseSpecific ? phaseId : undefined,
    });
    const rawFormCustomFields = useMemo(
      () => rawFormCustomFieldso,
      [rawFormCustomFieldso]
    );
    const selectedField = useMemo(() => selectedFieldp, [selectedFieldp]);
    const formCustomFields: IFlatCustomFieldWithIndex[] | undefined =
      useMemo(() => {
        return (
          rawFormCustomFields?.map((field, index) => ({
            ...field,
            index,
          })) || []
        );
    ...
    const isDirtyMemoized = useMemo(() => isDirty, [isDirty]);
    ...
    const closeSettings = useCallback((triggerAutosave?: boolean) => {
      setSelectedField(undefined);
    ...
    const handleAccessRightsClose = () =>
      setAccessRightsMessageIsVisible(false);
              ...
              <Box mt={`${stylingConsts.menuHeight}px`} display="flex">
                <Box width="210px">
                  <FormBuilderToolbox
                    ...
                    builderConfig={builderConfig}
                    move={move}
                    newFieldIndex={formCustomFields.length}
                  />
                </Box>
                <Box
                  flex="1.8"
...
const FormBuilderPage = ({
  ...
  const rawFormCustomFields = builderConfig.formCustomFields;

  if (isNilOrError(rawFormCustomFields) || isNilOrError(submissionCount)) {
    return <Spinner />;
  }
  ...
  // const rawFormCustomFields = useMemo(() => rawFormCustomFieldso, [rawFormCustomFieldso]);
  // const selectedField = useMemo(() => selectedFieldp, [selectedFieldp]);
  // const formCustomFields: IFlatCustomFieldWithIndex[] | undefined = useMemo(() => {
  //   return rawFormCustomFields?.map((field, index) => ({
  //     ...field,
  //     index,
  //   })) || [];
  // }, [rawFormCustomFields]);
...
FormBuilderPage.whyDidYouRender = {
  logOnDifferentValues: true,
  customName: 'FormBuilderPage',
};

// FormBuilderPage.whyDidYouRender = true;
